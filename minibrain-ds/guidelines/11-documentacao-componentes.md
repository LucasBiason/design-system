# Documentação de Componentes MiniBrain (Vanilla TS) — Regras Obrigatórias

## Princípio Fundamental: Use os Section Containers

Todas as docs pages MiniBrain **DEVEM usar as factory functions** em `src/components/docs/shared/sections/`. Cada seção é uma função `createDocsXxx(props): HTMLElement` que encapsula o layout, o wrapper card, os headings, os grids e a semântica. A docs page é apenas o **orquestrador** — constrói o DOM chamando essas factories e passando dados + **factory functions de preview**.
...
## Regras MiniBrain-Específicas

### Factory functions de preview

Em MiniBrain não há slots/snippets. Previews são passados como **factory functions** `() => HTMLElement`:
...
MiniBrain não tem reatividade automática. A docs page:
...
6. **`createDocsProps`** — 1 tabela única com 4 linhas: `ratio` (number, default 1), `content` (`HTMLElement`, obrigatório — **não existe** `asChild` no MiniBrain), `className` (string), e opcionalmente demais atributos HTML. Documentar a diferença frente às demais stacks (`content` em vez de `children`).
7. **`createDocsTokens`** — MiniBrain AspectRatio não usa tokens próprios. Documentar apenas os tokens aplicáveis **quando usado como placeholder**: `--radius` → `mbds-rounded-md`, `--border` → `mbds-border`, `--muted` → `mbds-bg-muted`. `tokens.note` no JSON explica que sem `content` o container é transparente. `customizationCode` instrui a aplicar classes no elemento passado como `content`, nunca no wrapper.
...
### Componentes de Visualização de Dados (padrão Chart) — MiniBrain

Componentes como **Chart** no MiniBrain usam a factory `createChart({ data, type, height, colors })` de `./chart` — API simplificada, **sem Recharts**. Apenas os tipos **`bar`** e **`line`** são suportados via prop `type`. O gráfico é SVG puro renderizado pela factory sem dependências externas. Categoria **Display**, translations em `docs/shared/content/chart/translations.json`.
...
5. **SVG puro — sem Recharts** — previews nas factories não usam Recharts. A factory `createChart` gera SVG puro. Não incluir dependências de `recharts` no MiniBrain.
...
7. **`createDocsAccessibility`** — `keyboardItems` com 4 entradas. No MiniBrain, `accessibilityLayer` não existe — acessibilidade por teclado é implementada com `tabIndex`, `aria-label`, `role="img"` e `<title>` SVG.

8. **`notes.tip3`** — nota crítica: "No MiniBrain, apenas os tipos `bar` e `line` são suportados via prop `type`. A API é simplificada: `createChart({ data, type, height, colors })`." Esta nota já existe em `notes.tip3` e deve ser renderizada de forma destacada (borda diferente ou ícone de aviso no callout).
...
- ❌ **NUNCA** copie classes `.mbds-*` dos containers para blocos `innerHTML` sem necessidade — herde via composição
