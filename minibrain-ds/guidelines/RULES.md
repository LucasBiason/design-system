# Rules — Design System MiniBrain (Vanilla TypeScript)
...
- **Estilos**: CSS standalone via classes `.mbds-*` definidas em `src/styles/components/*.css` (sem Tailwind) — nunca estilos inline arbitrários (exceto CSS custom properties dinâmicas)
...
- **Tipografia**: fonte do sistema definida no CSS base — usar classes `.mbds-text-*` / `.mbds-leading-*` (sem valores arbitrários ou classes utilitárias de outro framework)
