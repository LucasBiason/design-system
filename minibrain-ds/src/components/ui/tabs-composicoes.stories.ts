...
function makePanel(text: string): HTMLElement {
  ...
  p.className = 'mbds-text-body mbds-text-muted-foreground mbds-p-3 mbds-rounded-md mbds-border-default mbds-bg-card';
  p.textContent = text;
  return p;
}
...
function makeRichPanel(title: string, description: string): HTMLElement {
  ...
  wrap.className = 'mbds-stack mbds-p-4 mbds-rounded-md mbds-border-default mbds-bg-card';
...
export const Vertical: Story = {
  render: () => {
    ...
    if (list) {
      ...
      list.classList.add('mbds-shrink-0');
...
export const SubNavegacaoLine: Story = {
  render: () => {
    ...
    if (list) {
      ...
      list.classList.add('mbds-bg-transparent', 'mbds-w-full');
