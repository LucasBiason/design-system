...
const ICONS = {
  arrowUp: `<svg ${ICON_BASE_ATTRS} class="mbds-dt-icon"><path d="M12 19V5"/><path d="m5 12 7-7 7 7"/></svg>`,
  arrowDown: `<svg ${ICON_BASE_ATTRS} class="mbds-dt-icon"><path d="M12 5v14"/><path d="m19 12-7 7-7-7"/></svg>`,
  arrowUpDown: `<svg ${ICON_BASE_ATTRS} class="mbds-dt-icon mbds-dt-icon-muted"><path d="m21 16-4 4-4-4"/><path d="M17 20V4"/><path d="m3 8 4-4 4 4"/><path d="M7 4v16"/></svg>`,
  chevronLeft: `<svg ${ICON_BASE_ATTRS} class="mbds-dt-icon"><path d="m15 18-6-6 6-6"/></svg>`,
  chevronRight: `<svg ${ICON_BASE_ATTRS} class="mbds-dt-icon"><path d="m9 18 6-6-6-6"/></svg>`,
  chevronsLeft: `<svg ${ICON_BASE_ATTRS} class="mbds-dt-icon"><path d="m11 17-5-5 5-5"/><path d="m18 17-5-5 5-5"/></svg>`,
  chevronsRight: `<svg ${ICON_BASE_ATTRS} class="mbds-dt-icon"><path d="m6 17 5-5-5-5"/><path d="m13 17 5-5-5-5"/></svg>`,
  grip: `<svg ${ICON_BASE_ATTRS} class="mbds-dt-icon mbds-dt-icon-grip"><circle cx="9" cy="6" r="1"/><circle cx="9" cy="12" r="1"/><circle cx="9" cy="18" r="1"/><circle cx="15" cy="6" r="1"/><circle cx="15" cy="12" r="1"/><circle cx="15" cy="18" r="1"/></svg>`,
  pin: `<svg ${ICON_BASE_ATTRS} class="mbds-dt-icon"><path d="M12 17v5"/><path d="M9 10.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24V16a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-.76a2 2 0 0 0-1.11-1.79l-1.78-.9A2 2 0 0 1 15 10.76V7a1 1 0 0 1 1-1 2 2 0 0 0 0-4H8a2 2 0 0 0 0 4 1 1 0 0 1 1 1z"/></svg>`,
  pinOff: `<svg ${ICON_BASE_ATTRS} class="mbds-dt-icon"><path d="M12 17v5"/><path d="M15 9.34V6h1a2 2 0 0 0 0-4H7.89"/><path d="m2 2 20 20"/><path d="M9 9v1.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24V16a1 1 0 0 0 1 1h11"/></svg>`,
  search: `<svg ${ICON_BASE_ATTRS} class="mbds-dt-icon mbds-dt-icon-muted"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>`,
  settings: `<svg ${ICON_BASE_ATTRS} class="mbds-dt-icon"><path d="M20 7h-9"/><path d="M14 17H5"/><circle cx="17" cy="17" r="3"/><circle cx="7" cy="7" r="3"/></svg>`,
};
...
export function createDataTable<TData extends RowData>(
  ...
  root.className = 'mbds-data-table';
  if (className) root.classList.add(...className.split(' ').filter(Boolean));

  // Toolbar
  const toolbar = document.createElement('div');
  toolbar.dataset.slot = 'data-table-toolbar';
  toolbar.className = 'mbds-data-table-toolbar';

  // Scroll container
  const scrollContainer = document.createElement('div');
  scrollContainer.className = 'mbds-data-table-scroll';
  if (virtualized) {
  ...
  tableWrapper.classList.add('mbds-data-table-table-wrapper');
  if (enableColumnResizing || enableColumnOrdering || virtualized) {
    tableEl.classList.add('mbds-table-fixed');
  }
  ...
  pagFooter.className = 'mbds-data-table-pagination';
  ...
  function renderToolbar() {
    ...
    if (enableGlobalFilter) {
      const wrap = document.createElement('div');
      wrap.className = 'mbds-data-table-search';
      wrap.appendChild(svgEl(ICONS.search));
      const input = createInput({
        type: 'search',
        placeholder: globalFilterPlaceholder,
        value: globalFilter,
        class: 'mbds-data-table-search-input',
      });
    ...
    if (enableColumnVisibility) {
      const visBtn = createButton({
        variant: 'outline',
        size: 'sm',
        class: 'mbds-data-table-columns-btn',
        ariaLabel: L.columns,
      });
      ...
      menu.className = 'mbds-data-table-columns-menu';
      menu.hidden = true;
      ...
      function rebuildMenu() {
        ...
        header.className = 'mbds-data-table-columns-menu-header';
        ...
        for (const column of table.getAllLeafColumns()) {
          ...
          row.className = 'mbds-data-table-columns-menu-row';
          ...
          text.className = 'mbds-data-table-columns-menu-label';
          ...
          if (enableColumnPinning) {
            ...
            pinBtn.type = 'button';
            pinBtn.className = 'mbds-data-table-pin-btn';
      ...
      const wrap = document.createElement('div');
      wrap.className = 'mbds-data-table-columns-wrap';
  ...
  function renderHeader() {
    ...
    for (const headerGroup of table.getHeaderGroups()) {
      ...
      for (const header of headerGroup.headers) {
        ...
        th.className = 'mbds-data-table-th';
        th.scope = 'col';
        ...
        if (!header.isPlaceholder) {
          ...
          inner.className = 'mbds-data-table-th-inner';
          ...
          if (col.id === '__select__') {
            ...
            btn.type = 'button';
            btn.className = 'mbds-data-table-sort-btn';
            ...
            div.className = 'mbds-data-table-th-label';
            div.textContent = lbl;
          ...
          if (enableColumnResizing && col.getCanResize()) {
            ...
            handle.className = 'mbds-data-table-resize-handle';
    ...
    if (hasFilters) {
      const tr = createTableRow();
      tr.classList.add('mbds-data-table-filter-row');
      for (const header of table.getHeaderGroups()[0]?.headers ?? []) {
        ...
        if (col.getCanFilter() && meta) {
          ...
          if (meta.type === 'select') {
            ...
            select.className = 'mbds-data-table-filter-select';
            ...
            const input = createInput({
              ...
              class: 'mbds-data-table-filter-input',
            });
  ...
  function buildEditableCell<TVal>(
    ...
    container.className = 'mbds-data-table-editable';

    function renderView() {
      container.replaceChildren();
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'mbds-data-table-edit-btn';
      btn.setAttribute('aria-label', L.edit(label));
      if (initial == null || initial === '') {
        const dash = document.createElement('span');
        dash.className = 'mbds-text-muted-foreground';
        dash.textContent = '—';
    ...
    function renderEdit() {
      ...
      const input = createInput({
        ...
        class: 'mbds-data-table-edit-input',
      });
  ...
  function rerenderBody() {
    ...
    if (rows.length === 0) {
      ...
      td.className = 'mbds-data-table-empty';
  ...
  function renderPagination() {
    ...
    pageInd.className = 'mbds-text-muted-foreground';
    pageInd.textContent = `${L.page} ${pageIndex + 1} ${L.pageOf} ${Math.max(pageCount, 1)}`;

    // Nav buttons
    const navWrap = document.createElement('div');
    navWrap.className = 'mbds-data-table-pagination-nav';
