...
export const MobileBottomPanel: Story = {
  ...
  render: () => {
    ...
    list.className = 'mbds-grid mbds-text-body';
    list.dataset.cols = '3';
    ...
    for (const label of actions) {
      ...
      item.type = 'button';
      item.className = 'mbds-stack mbds-rounded-md mbds-border-default mbds-hover-bg-accent';
      item.dataset.spacing = 'xs';
      item.dataset.align = 'center';
      item.style.padding = '0.75rem';
      const dot = document.createElement('div');
      dot.className = 'mbds-rounded-full mbds-bg-muted';
...
export const WithLongScrollContent: Story = {
  ...
  render: () => {
    ...
    longBody.className = 'mbds-stack mbds-text-body mbds-text-muted-foreground';
