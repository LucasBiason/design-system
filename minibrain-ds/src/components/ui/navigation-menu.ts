...
// Visual: classes .mbds-navigation-menu-* (zero Tailwind/basecoat-css).
...
function createChevronSvg(): SVGSVGElement {
  ...
  svg.setAttribute('class', 'mbds-navigation-menu-chevron');
...
export function createNavigationMenu(
  ...
  items.forEach((item, idx) => {
    ...
    if (!item.children || item.children.length === 0) {
      ...
      a.href = item.href ?? '#';
      a.className = 'mbds-navigation-menu-link';
      ...
      trigger.type = 'button';
      trigger.className = 'mbds-navigation-menu-trigger';
      ...
      content.className = 'mbds-navigation-menu-content';
      ...
      item.children.forEach((child) => {
        ...
        childA.className = 'mbds-navigation-menu-child';
        childA.setAttribute('role', 'menuitem');

        const labelEl = document.createElement('div');
        labelEl.className = 'mbds-navigation-menu-child-label';
        labelEl.textContent = child.label;
        childA.appendChild(labelEl);

        if (child.description) {
          const descEl = document.createElement('p');
          descEl.className = 'mbds-navigation-menu-child-description';
      ...
      itemWrapper.className = 'mbds-navigation-menu-item';
