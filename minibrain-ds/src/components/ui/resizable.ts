...
// Visual: classes .mbds-resizable-* (zero Tailwind/basecoat-css).
...
export function createResizablePanel(options: ResizablePanelOptions): HTMLElement {
  ...
  panels.forEach((panel, i) => {
    ...
    panelEl.className = 'mbds-resizable-panel';
    ...
    if (i < panels.length - 1) {
      ...
      handle.className = 'mbds-resizable-handle';

      // Grip (alça com pontos) — SVG via createElementNS (sem innerHTML).
      const grip = document.createElement('div');
      grip.className = 'mbds-resizable-grip';
