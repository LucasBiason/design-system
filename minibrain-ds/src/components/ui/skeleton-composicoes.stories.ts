...
function makeSkeleton(className: string): HTMLElement {
  ...
  for (const t of tokens) {
    ...
    else if (t === 'rounded-full') rounded.push('mbds-rounded-full');
    else if (t === 'rounded-md') rounded.push('mbds-rounded-md');
...
export const CardDePerfil: Story = {
  name: 'Card de Perfil',
  render: () => {
    const wrap = loadingContainer('Carregando card de perfil', 'mbds-max-w-sm');
  ...
  play: async ({ canvasElement, step }) => {
    await step('Card de perfil tem 1 avatar circular + 2 linhas', async () => {
      ...
      await expect(skeletons[0]).toHaveClass('mbds-rounded-full');
    });
...
export const ListaComAvatar: Story = {
  ...
  render: () => {
    const wrap = loadingContainer('Carregando lista de pedidos', 'mbds-stack mbds-max-w-md');
...
export const ImagemEmAspectRatio: Story = {
  ...
  render: () => {
    const wrap = loadingContainer('Carregando imagem', 'mbds-max-w-md');
...
export const Paragrafo: Story = {
  name: 'Parágrafo',
  render: () => {
    const wrap = loadingContainer('Carregando parágrafo', 'mbds-stack mbds-max-w-md');
