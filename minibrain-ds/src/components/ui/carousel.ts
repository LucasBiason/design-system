...
// Visual: classes .mbds-carousel-* (zero Tailwind/basecoat-css).
...
export function createCarousel(options: CarouselOptions): HTMLElement {
  ...
  root.className = 'mbds-carousel';
  if (options.class) root.classList.add(...options.class.split(' ').filter(Boolean));

  // Overflow wrapper
  const overflow = document.createElement('div');
  overflow.className = 'mbds-carousel-overflow';

  // Track
  const track = document.createElement('div');
  track.className = 'mbds-carousel-track';
  track.dataset.slot = 'carousel-track';

  items.forEach((item, i) => {
    const slide = document.createElement('div');
    slide.className = 'mbds-carousel-slide';
  ...
  prevBtn.type = 'button';
  prevBtn.className = 'mbds-carousel-button mbds-carousel-button-prev';
  ...
  nextBtn.type = 'button';
  nextBtn.className = 'mbds-carousel-button mbds-carousel-button-next';
