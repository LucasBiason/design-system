...
function buildField(opts: FieldOpts): HTMLElement {
  ...
  if (hasCounter || hasHelp) {
    ...
    row.className = 'mbds-cluster mbds-text-caption mbds-text-muted-foreground';
    ...
    if (hasCounter) {
      ...
      counter.className = 'tabular-nums mbds-shrink-0';
...
form.className = 'mbds-stack mbds-w-full mbds-max-w-md';
...
form.dataset.spacing = 'md';
                ...
                const result = document.createElement('p');
                result.className = 'mbds-text-caption mbds-text-muted-foreground';
                result.setAttribute('aria-live', 'polite');
      ...
      case 'propriedades': {
        const interfaceCode = `// createTextarea(options) — MiniBrain
export type TextareaOptions = {
  ...
  name?: string;
        ...
        return createDocsProps({
          ...
          tables: [
            {
              title: 'createTextarea(options) — MiniBrain',
              cols: propsCols,
              items: [
                {
                  name: 'placeholder',
          ...
          extensibilityNotes:
            'O factory MiniBrain é um wrapper enxuto de `<textarea>` HTML nativo e diverge das libs upstream nos seguintes pontos: ' +
