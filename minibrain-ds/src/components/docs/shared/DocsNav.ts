...
export function createDocsNav(props: DocsNavProps): DocsNavHandle {
  ...
  root.className = 'mbds-docs-nav';

  const buttons = new Map<string, HTMLButtonElement>();

  for (const group of props.groups) {
    const groupEl = document.createElement('div');
    groupEl.className = 'mbds-docs-nav-group';

    const label = document.createElement('p');
    label.className = 'mbds-docs-nav-label';
    label.textContent = group.label;
    groupEl.appendChild(label);

    const list = document.createElement('ul');
    list.className = 'mbds-docs-nav-list';
    ...
    for (const section of group.sections) {
      ...
      btn.type = 'button';
      btn.className = 'mbds-docs-nav-button';
