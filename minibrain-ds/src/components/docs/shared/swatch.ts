...
export function createSwatch({ token, orientation, value = '' }: SwatchOptions): SwatchHandles {
  if (orientation === 'vertical') {
    ...
    item.className = 'mbds-miniswatch';

    const chip = document.createElement('span');
    chip.className = 'mbds-miniswatch-chip';
    chip.style.setProperty('--swatch-color', `var(--${token})`);

    const name = document.createElement('span');
    name.className = 'mbds-miniswatch-name';
  ...
  btn.type = 'button';
  btn.className = 'mbds-swatch';

  const color = document.createElement('span');
  color.className = 'mbds-swatch-color';
  color.style.setProperty('--swatch-color', `var(--${token})`);

  const meta = document.createElement('div');
  meta.className = 'mbds-swatch-meta';

  const tokenLabel = document.createElement('span');
  tokenLabel.className = 'mbds-swatch-token';
  tokenLabel.textContent = `--${token}`;

  const valueLabel = document.createElement('span');
  valueLabel.className = 'mbds-swatch-value';
