...
export function createDocsRelated(props: DocsRelatedProps): HTMLElement {
  ...
  props.items.forEach(item => {
    ...
    // Implementado como classe própria .mbds-related-card em vez de usar
    // .mbds-button-outline porque o layout difere (vertical, multi-linha,
    // padding maior, sem white-space:nowrap nem inline-flex centralizado).
    a.className = 'mbds-related-card';
    ...
    a.innerHTML = `
      <span class="mbds-related-card-title">${sanitizeHtml(item.name)}</span>
      <span class="mbds-related-card-description">${sanitizeHtml(item.description)}</span>`;
    grid.appendChild(a);
  });
