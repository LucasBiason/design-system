...
const priorityBadgeClass = (p: string): string =>
  ({ Alta: 'mbds-badge-high', Média: 'mbds-badge-medium', Baixa: 'mbds-badge-low', High: 'mbds-badge-high', Medium: 'mbds-badge-medium', Low: 'mbds-badge-low' } as Record<string, string>)[p] ?? 'mbds-badge-outline';
...
export function createDocsTestes(props: DocsTestesProps): HTMLElement {
  ...
  funcBlock.className = 'mbds-stack';
  funcBlock.dataset.spacing = 'sm';

  const funcTitle = document.createElement('h3');
  funcTitle.className = 'mbds-text-base mbds-font-semibold';
  funcTitle.textContent = props.functional.title;
  funcBlock.appendChild(funcTitle);

  const funcWrapper = createCard({ className: 'mbds-p-4 mbds-overflow-x' });
  const { wrapper: funcTableWrapper, table: funcTable } = createTable('mbds-w-full mbds-text-body');

  const funcThead = createTableHeader();
  const funcHeaderRow = createTableRow('mbds-border-b mbds-bg-muted-soft');
  funcHeaderRow.appendChild(createTableHead(props.functional.cols.action, 'mbds-p-2 mbds-font-semibold'));
  funcHeaderRow.appendChild(createTableHead(props.functional.cols.result, 'mbds-p-2 mbds-font-semibold'));
  funcHeaderRow.appendChild(createTableHead(props.functional.cols.priority, 'mbds-p-2 mbds-font-semibold'));
  ...
  props.functional.items.forEach(item => {
    ...
    row.appendChild(createTableCell(item.action, 'mbds-p-2'));
    row.appendChild(createTableCell(item.result, 'mbds-p-2 mbds-text-muted-foreground'));
    const priorityCell = createTableCell('', 'mbds-p-2 mbds-font-medium');
  ...
  a11yBlock.className = 'mbds-stack';
  a11yBlock.dataset.spacing = 'sm';

  const a11yTitle = document.createElement('h3');
  a11yTitle.className = 'mbds-text-base mbds-font-semibold';
  ...
  props.accessibility.items.forEach(item => {
    const card = createCard({ className: 'mbds-bg-muted-soft mbds-border-none mbds-shadow-none mbds-p-2 mbds-stack' });
    card.dataset.spacing = 'xs';
    const top = document.createElement('div');
    top.className = 'mbds-row';
    ...
    criterionSpan.className = 'mbds-text-body mbds-font-medium';
    criterionSpan.textContent = item.criterion;
    top.append(levelKbd, criterionSpan);
    const howP = document.createElement('p');
    howP.className = 'mbds-text-body mbds-text-muted-foreground';
  ...
  visualBlock.className = 'mbds-stack';
  visualBlock.dataset.spacing = 'sm';

  const visualTitle = document.createElement('h3');
  visualTitle.className = 'mbds-text-base mbds-font-semibold';
  visualTitle.textContent = props.visual.title;
  visualBlock.appendChild(visualTitle);

  const visualWrapper = createCard({ className: 'mbds-p-4 mbds-overflow-x' });
  const { wrapper: visualTableWrapper, table: visualTable } = createTable('mbds-w-full mbds-text-body');

  const visualThead = createTableHeader();
  const visualHeaderRow = createTableRow('mbds-border-b mbds-bg-muted-soft');
  visualHeaderRow.appendChild(createTableHead(props.visual.cols.story, 'mbds-p-2 mbds-font-semibold'));
  visualHeaderRow.appendChild(createTableHead(props.visual.cols.priority, 'mbds-p-2 mbds-font-semibold'));
  visualThead.appendChild(visualHeaderRow);

  const visualTbody = createTableBody();
  props.visual.items.forEach(item => {
    const row = createTableRow('mbds-border-b mbds-hover-bg-muted-faint');
    row.appendChild(createTableCell(item.story, 'mbds-p-2'));
    const priorityCell = createTableCell('', 'mbds-p-2 mbds-font-medium');
    priorityCell.appendChild(createBadge({ text: item.priority, className: priorityBadgeClass(item.priority) }));
    row.appendChild(priorityCell);
