...
function buildProductCardPreview(): HTMLElement {
  ...
  price.className = 'mbds-text-lead mbds-font-semibold';
  price.textContent = t('demonstration.labels.productPrice');
  const stock = document.createElement('p');
  stock.className = 'mbds-text-body mbds-text-muted-foreground mbds-mt-1';
...
function buildProfileCardPreview(): HTMLElement {
  ...
  textWrap.className = 'mbds-min-w-0';
...
function buildMetricCardPreview(): HTMLElement {
  const card = createCard({ className: 'mbds-w-full mbds-max-w-xs' });
  const header = createCardHeader();
  ...
  trend.className = 'mbds-text-body mbds-text-muted-foreground mbds-mt-1';
...
function buildSmallCardPreview(): HTMLElement {
  const card = createCard({ size: 'sm', className: 'mbds-w-full mbds-max-w-xs' });
  const header = createCardHeader();
  header.appendChild(createCardTitle({ text: t('demonstration.labels.metricTitle'), level: 4 }));
  const content = createCardContent();
  const value = document.createElement('p');
  value.className = 'mbds-text-lead mbds-font-semibold';
  value.textContent = t('demonstration.labels.metricValue');
...
export function createCardDocs(): HTMLElement {
      ...
      case 'demonstracao':
        return createDocsDemonstration({
          ...
          demoFactory: () => {
            ...
            wrap.className = 'mbds-w-full mbds-grid';
            wrap.dataset.cols = '2';
          ...
          pairs: [
            {
              ...
              dontPreviewFactory: () => {
                const card = createCard({ className: 'mbds-w-full mbds-max-w-sm' });
                const content = createCardContent({ className: 'mbds-p-4' });
  ...
  createCardFooter,
          ...
          secondaryCode: `const card = createCard({ className: 'mbds-w-full mbds-max-w-sm' });
...
card.append(header, content, footer);`,
      ...
      case 'variantes': {
        ...
        const codeDefault = `const card = createCard({ className: 'mbds-w-full mbds-max-w-sm' });
const header = createCardHeader();
header.appendChild(createCardTitle({ text: 'Cadeira Gamer Pro', level: 3 }));
...
const img = document.createElement('img');
img.src = '/produto.jpg';
img.className = 'mbds-w-full object-cover';
card.append(img, header);`;
          ...
          items: [
            ...
            },
            {
              ...
              code:
                ...
                `const content = createCardContent();\n` +
                `const value = document.createElement('p');\n` +
                `value.className = 'text-2xl mbds-font-semibold';\n` +
