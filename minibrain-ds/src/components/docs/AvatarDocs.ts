...
function buildUserIconSvg(): SVGSVGElement {
  ...
  svg.setAttribute('class', 'mbds-icon mbds-text-muted-foreground');
...
function buildGroupAvatar(): HTMLElement {
  ...
  more.appendChild(createAvatarFallback({ text: '+3', className: 'mbds-text-caption' }));
  applyRing(more, true);
...
function buildStatusAvatar(): HTMLElement {
  ...
  status.className = 'mbds-rounded-full mbds-bg-primary';
          ...
          items: [
            ...
            },
            {
              ...
              code:
                ...
                `fallback.setAttribute('aria-label', 'Usuário genérico');\n` +
                `const svg = createUserIconSvg(); // mbds-icon mbds-text-muted-foreground, aria-hidden\n` +
