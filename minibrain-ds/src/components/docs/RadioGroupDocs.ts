...
function buildRadioGroupWithLegend(opts: {
  ...
  if (ariaInvalid) {
    ...
    group.querySelectorAll<HTMLButtonElement>('[data-slot="radio-group-item"]').forEach((btn) => {
      ...
      btn.classList.add('mbds-border-destructive', 'ring-destructive/20');
    });
  }
...
group.setAttribute('aria-labelledby', 'payment-legend');`,
      ...
      case 'composicoes': {
        ...
        const buildInForm = () => {
          ...
          form.className = 'mbds-stack mbds-p-4 mbds-border-default mbds-rounded-lg';
          form.dataset.spacing = 'md';
          form.style.width = '20rem';
          form.noValidate = true;

          const fs = document.createElement('fieldset');
          fs.className = 'mbds-stack mbds-border-none mbds-p-0 mbds-m-0';
          fs.dataset.spacing = 'sm';

          const legend = document.createElement('legend');
          legend.className = 'mbds-text-body mbds-font-semibold mbds-mb-2';
          legend.textContent = 'Forma de pagamento';
...
const fs = document.createElement('fieldset');
fs.className = 'mbds-stack border-0 mbds-p-0 mbds-m-0';
const legend = document.createElement('legend');
legend.className = 'mbds-text-body mbds-font-semibold mbds-mb-2';
legend.textContent = 'Forma de pagamento';
fs.appendChild(legend);
...
export type RadioGroupOptions = {
  ...
  class?: string;
        ...
        return createDocsProps({
          ...
          tables: [
            {
              title: 'createRadioGroup(options) — MiniBrain',
              cols: propsCols,
              items: [
                { name: 'name',          type: 'string',                            defaultValue: '—',      required: 'Sim', description: stripHtml(t('props.table.name.description')) + ' Obrigatório no MiniBrain (não-controlado, participa do `FormData`).' },
                ...
                { name: 'class',         type: 'string',                            defaultValue: '—',      required: 'Não', description: 'Classes .mbds-* adicionais no `<fieldset>` raiz.' },
              ],
            },
          ],
          interfaceCode,
