    ...
    switch (id) {
      ...
      case 'composicoes': {
        ...
        const codeWithPrefix =
          ...
          `row.className = 'mbds-cluster mbds-rounded-md mbds-border-default border-input focus-within:ring-2 focus-within:ring-ring/50 mbds-overflow-hidden';\n` +
          `const prefix = document.createElement('span');\n` +
          `prefix.className = 'mbds-cluster mbds-px-3 mbds-text-body mbds-text-muted-foreground mbds-bg-muted border-r border-input mbds-shrink-0';\n` +
          `prefix.textContent = 'https://';\n` +
          `const input = createInput({ type: 'url', id: 'input-url', placeholder: 'meusite.com' });\n` +
          `input.className = 'border-0 focus-visible:ring-0 focus-visible:ring-offset-0 mbds-flex-1';\n` +
          ...
          items: [
            ...
            },
            {
              ...
              previewFactory: () => {
                ...
                const prefix = document.createElement('span');
                prefix.className = 'mbds-cluster mbds-px-2 mbds-text-body mbds-text-muted-foreground mbds-bg-muted mbds-shrink-0';
                prefix.style.borderRight = '1px solid var(--input)';
                prefix.textContent = 'https://';
                const input = createInput({ type: 'url', id: 'input-url', placeholder: 'meusite.com' });
                input.className = 'mbds-flex-1';
...
export type InputOptions = {
  ...
  name?: string;        // name do campo
        ...
        return createDocsProps({
          ...
          extensibilityNotes: 'O Input MiniBrain aceita todos os atributos HTML nativos do input via setAttribute() após a criação. Use a prop class para customizações pontuais com tokens do projeto.',
        });
      }
