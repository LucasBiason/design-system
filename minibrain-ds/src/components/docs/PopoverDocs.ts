...
createPopover({ trigger, content });`;
        ...
        function buildEditProfilePreview(): HTMLElement {
          ...
          heading.className = 'mbds-text-body mbds-font-medium mbds-leading-none';
      ...
      case 'propriedades': {
        const interfaceCode = `// createPopover(options) — factory custom MiniBrain
          ...
          tables: [
            {
              ...
              items: [
                ...
                { name: 'side',         type: "'top' | 'bottom' | 'left' | 'right'", defaultValue: "'bottom'",  required: 'Não', description: stripHtml(t('props.table.side.description')) + ' NOTA: factory MiniBrain não faz auto-flip por colisão.' },
                ...
                { name: 'open',         type: 'boolean',                             defaultValue: '—',         required: 'Não', description: stripHtml(t('props.table.open.description')) + ' NOTA: factory MiniBrain só suporta estado uncontrolled — observe via onOpenChange.' },
                { name: 'modal',        type: 'boolean',                             defaultValue: 'false',     required: 'Não', description: stripHtml(t('props.table.modal.description')) + ' NOTA: factory MiniBrain não implementa focus trap nem scroll lock.' },
                { name: 'sideOffset',   type: 'number',                              defaultValue: '8',         required: 'Não', description: stripHtml(t('props.table.sideOffset.description')) + ' NOTA: factory MiniBrain usa gap fixo (8px).' },
              ],
            },
          ],
          interfaceCode,
