...
function buildSheetDemo(opts: SheetDemoOptions): HTMLElement {
  ...
  body.className = 'mbds-stack mbds-text-body mbds-text-muted-foreground';
          ...
          pairs: [
            {
              ...
              doPreviewFactory: () => {
                const wrap = document.createElement('div');
                wrap.className = 'mbds-stack mbds-text-body';
  ...
  footer,
      ...
      case 'composicoes': {
        const buildSecondaryNavBody = () => {
          ...
          nav.className = 'mbds-stack mbds-px-4';
          nav.dataset.spacing = 'xs';
          ['Dashboard', 'Projetos', 'Equipe', 'Configurações', 'Faturas'].forEach((label) => {
            const a = document.createElement('a');
            a.href = '#';
            a.className = 'mbds-rounded-md mbds-text-body mbds-hover-bg-accent';
        ...
        const buildMobileActionsBody = () => {
          ...
          grid.className = 'mbds-grid mbds-px-4 mbds-text-body';
          grid.dataset.cols = '3';
          grid.dataset.spacing = 'sm';
          ['Compartilhar', 'Editar', 'Excluir', 'Arquivar', 'Mover', 'Copiar link'].forEach((label) => {
            const btn = document.createElement('button');
            btn.type = 'button';
            btn.className = 'mbds-stack mbds-rounded-md mbds-border-default mbds-hover-bg-accent';
        ...
        const buildLongBody = () => {
          const wrap = document.createElement('div');
          wrap.className = 'mbds-stack mbds-text-body mbds-text-muted-foreground';
          wrap.dataset.spacing = 'sm';
          for (let i = 1; i <= 14; i++) {
            const p = document.createElement('p');
...
createSheet({
  ...
  footer.dataset.spacing = 'xs';
      ...
      case 'propriedades': {
        const interfaceCode = `// createSheet(options) — factory custom MiniBrain
          ...
          tables: [
            {
              ...
              items: [
                ...
                { name: 'open',            type: 'boolean',                                           defaultValue: '—',       required: 'Não', description: 'NÃO SUPORTADO pela factory MiniBrain — estado é interno (uncontrolled). Use onOpenChange para observar mudanças.' },
                { name: 'defaultOpen',     type: 'boolean',                                           defaultValue: 'false',   required: 'Não', description: 'NÃO SUPORTADO pela factory MiniBrain — para abrir programaticamente, chame `trigger.click()`.' },
