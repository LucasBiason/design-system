...
function createPreviewImage(src: string, alt: string): HTMLImageElement {
  ...
  img.loading = 'lazy';
  img.decoding = 'async';
  img.className = 'mbds-w-full mbds-rounded-md';
...
function createPreviewIframe(src: string, title: string): HTMLIFrameElement {
  ...
  f.src = src;
  f.title = title;
  f.className = 'mbds-w-full mbds-rounded-md';
  f.style.height = '100%';
  f.style.border = '0';
  f.loading = 'lazy';
  return f;
...
function createPreviewVideo(poster: string, captionLabel: string): HTMLVideoElement {
  ...
  v.poster = poster;
  v.controls = true;
  v.className = 'mbds-w-full mbds-rounded-md';
...
export function createAspectRatioDocs(): HTMLElement {
    ...
    switch (id) {
      ...
      case 'anatomia':
        return createDocsAnatomy({
          ...
          structureCode:
            `<div class="relative mbds-w-full" style="padding-bottom: 56.25%">   // Root — wrapper com ratio\n` +
            `  <div class="absolute inset-0">                               // Inner — preenche o container\n` +
            `    <img class="object-cover mbds-w-full h-full mbds-rounded-md" />      // Slot — conteúdo filho\n` +
            `  </div>\n` +
            `</div>`,
        });
  ...
  img.style.height = '100%';
              ...
              dontPreviewFactory: () => {
                ...
                img.loading = 'lazy';
                img.decoding = 'async';
                img.className = 'mbds-w-full mbds-rounded-md mbds-bg-muted';
  ...
  img.style.height = '100%';
              ...
              dontPreviewFactory: () => {
                ...
                return createAspectRatio({
                  ratio: 4 / 3,
                  content: img,
                  className: 'mbds-rounded-md mbds-overflow-hidden',
                });
              },
            },
          ],
      ...
      case 'importacao':
        return createDocsImport({
          ...
          secondaryCode:
            ...
            `img.className = 'object-cover mbds-w-full h-full mbds-rounded-md';\n\n` +
  ...
  className?: string;
        ...
        return createDocsProps({
          ...
          extensibilityNotes:
            ...
            ' Nota MiniBrain: a factory aceita content: HTMLElement em vez de children e não suporta asChild.',
        });
      }

      case 'tokens': {
        const customizationCode = `/* AspectRatio não aplica tokens próprios. Aplique no filho: */
<img class="mbds-rounded-md object-cover mbds-w-full h-full" />

/* Placeholder opcional quando o conteúdo ainda não carregou: */
<div class="absolute inset-0 mbds-bg-muted mbds-rounded-md" aria-hidden="true"></div>`;
