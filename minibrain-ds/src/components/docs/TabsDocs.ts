...
function textPanel(text: string, extraClass = ''): HTMLElement {
  ...
  div.className = ['mbds-text-body mbds-text-muted-foreground mbds-p-2 mbds-rounded-md mbds-border-default mbds-bg-card', extraClass].filter(Boolean).join(' ');
...
function svgIcon(builder: (svg: SVGSVGElement) => void): SVGSVGElement {
  ...
  svg.setAttribute('class', 'mbds-icon mbds-shrink-0');
  builder(svg);
  return svg;
}
...
function richPanel(title: string, description: string): HTMLElement {
  ...
  wrap.className = 'mbds-p-4 mbds-rounded-md mbds-border-default mbds-bg-card mbds-stack';
  wrap.dataset.spacing = 'sm';
  const h = document.createElement('h3');
  h.className = 'mbds-text-body mbds-font-semibold';
  h.textContent = title;
...
function buildDemoTabs(): HTMLElement {
  // Factory custom MiniBrain NÃO suporta props `variant` nem `orientation` nem `activationMode`.
          ...
          pairs: [
            {
              ...
              doPreviewFactory: () => {
                const r = createTabs({
                  defaultValue: 'overview',
                  class: 'mbds-w-full mbds-max-w-xs',
                  items: [
...
`createTabs({
  ...
  items: [
    ...
    { value: 'examples',   label: 'Exemplos',     content: panelEl },
        ...
        const codeLine =
`// MiniBrain: factory custom NÃO expõe variant="line".
...
const list = root.querySelector('[role="tablist"]');
list?.classList.add('mbds-border-b', 'mbds-bg-transparent'); list?.style.borderRadius = '0';`;
        const codeVertical =
`// MiniBrain: factory custom NÃO expõe orientation="vertical".
// Para orientação vertical, envolver em flex e reaplicar o layout.
const root = createTabs({ defaultValue: 'overview', items: [...] });
root.classList.add('mbds-cluster');
const list = root.querySelector('[role="tablist"]');
list?.classList.add('mbds-stack'); list?.style.height = 'auto';
          ...
          items: [
            {
              ...
              previewFactory: () => {
                const r = createTabs({
                  defaultValue: 'overview',
                  class: 'mbds-w-full mbds-max-w-md',
                  items: [
                    { value: 'overview',   label: t('demonstration.labels.overview'),   content: textPanel(t('demonstration.labels.overviewContent')) },
                    { value: 'properties', label: t('demonstration.labels.properties'), content: textPanel(t('demonstration.labels.propertiesContent')) },
                    { value: 'examples',   label: t('demonstration.labels.examples'),   content: textPanel(t('demonstration.labels.examplesContent')) },
            ...
            },
            {
              ...
              description: stripHtml(t('variants.styles.line')) + ' — MiniBrain: aplicar via .class manualmente (factory não expõe variant).',
              code: codeLine,
              previewFactory: () => {
                const r = createTabs({
                  defaultValue: 'overview',
                ...
                if (list) {
                  list.classList.add('mbds-border-b', 'mbds-bg-transparent', 'mbds-w-full'); list.style.borderRadius = '0'; list.style.justifyContent = 'flex-start';
            ...
            },
            {
              ...
              description: stripHtml(t('variants.styles.vertical')) + ' — MiniBrain: aplicar via .class manualmente (factory não expõe orientation).',
              code: codeVertical,
              previewFactory: () => {
                const r = createTabs({
                  defaultValue: 'overview',
                  class: 'mbds-w-full mbds-max-w-md mbds-cluster',
                  items: [
                    { value: 'overview',   label: t('demonstration.labels.overview'),   content: textPanel(t('demonstration.labels.overviewContent')) },
                    { value: 'properties', label: t('demonstration.labels.properties'), content: textPanel(t('demonstration.labels.propertiesContent')) },
                    { value: 'examples',   label: t('demonstration.labels.examples'),   content: textPanel(t('demonstration.labels.examplesContent')) },
                ...
                const list = r.querySelector('[role="tablist"]') as HTMLElement | null;
                if (list) {
                  list.classList.add('mbds-stack', 'mbds-shrink-0'); list.style.height = 'auto'; list.style.alignItems = 'stretch';
                  list.setAttribute('aria-label', t('demonstration.title'));
                  list.setAttribute('aria-orientation', 'vertical');
                }
                return r;
...
const root = createTabs({ defaultValue: 'profile', class: 'mbds-w-full', items });
...
const root = createTabs({ defaultValue: 'inbox', class: 'mbds-w-full', items });
const badgeMap = {
...
`const root = createTabs({
  defaultValue: 'profile',
  class: 'mbds-w-full mbds-cluster',
  items: [
    { value: 'profile',  label: 'Perfil',    content: panelEl },
    { value: 'account',  label: 'Conta',     content: panelEl },
    { value: 'security', label: 'Segurança', content: panelEl },
...
if (list) {
  list.classList.add('mbds-stack', 'mbds-shrink-0'); list.style.height = 'auto'; list.style.alignItems = 'stretch'; list.style.minWidth = '10rem';
  list.setAttribute('aria-orientation', 'vertical');
          ...
          items: [
            {
              ...
              previewFactory: () => {
                ...
                const r = createTabs({ defaultValue: 'profile', class: 'mbds-w-full', items });
                items.forEach((item) => {
            ...
            },
            {
              ...
              previewFactory: () => {
                ...
                const r = createTabs({ defaultValue: 'inbox', class: 'mbds-w-full', items });
                items.forEach((item) => {
            ...
            },
            {
              ...
              previewFactory: () => {
                ...
                const r = createTabs({ defaultValue: 'profile', class: 'mbds-w-full mbds-cluster', items });
                const list = r.querySelector('[role="tablist"]') as HTMLElement | null;
                if (list) {
                  list.classList.add('mbds-stack', 'mbds-shrink-0'); list.style.height = 'auto'; list.style.alignItems = 'stretch'; list.style.minWidth = '10rem';
                  list.setAttribute('aria-orientation', 'vertical');
            ...
            },
            {
              ...
              previewFactory: () => {
                ...
                const r = createTabs({ defaultValue: 'all', class: 'mbds-w-full', items });
                const list = r.querySelector('[role="tablist"]') as HTMLElement | null;
                if (list) {
                  list.classList.add('mbds-border-b', 'mbds-bg-transparent', 'mbds-w-full'); list.style.borderRadius = '0'; list.style.justifyContent = 'flex-start';
                  list.setAttribute('aria-label', 'Filtros de listagem');
...
export function createTabs(options: TabsOptions): HTMLElement;`;
        ...
        const DIVERGENCE = ' (MiniBrain: não suportado pela factory custom — aplicar via .class manualmente)';
        ...
        return createDocsProps({
          ...
          tables: [
            {
              ...
              items: [
                ...
                { name: 'activationMode', type: '"automatic" | "manual"',     defaultValue: '"automatic"', required: 'Não', description: stripHtml(t('props.table.activationMode.description')) + ' MiniBrain: apenas "automatic" implementado.' },
          ...
          extensibilityNotes: 'MiniBrain usa factory custom. Para variantes "line" e "vertical", aplicar classes utilitárias no elemento [role="tablist"] após criar o componente — ver seção Variantes.',
        });
      }
      ...
      case 'notas':
        return createDocsNotes({
          ...
          items: [
            ...
            // Divergência idiomática MiniBrain (3ª camada — ver também DocsProps e composicoes story)
            { title: '', content: 'MiniBrain: factory custom NÃO expõe props variant, orientation ou activationMode="manual". Para visual line/vertical, aplicar utility classes no elemento [role="tablist"] após criar (ver Variantes e tabs-composicoes.stories.ts).' },
          ],
        });
