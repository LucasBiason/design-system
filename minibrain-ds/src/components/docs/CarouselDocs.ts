...
function buildSlide(label: string, tone: 'muted' | 'primary' = 'muted'): HTMLElement {
  const card = createCard({
    className: tone === 'primary'
      ? 'mbds-w-full mbds-cluster mbds-bg-primary-soft'
      : 'mbds-w-full mbds-cluster mbds-bg-muted-soft',
  });
  ...
  span.className = 'mbds-font-semibold mbds-text-foreground';
          ...
          pairs: [
            {
              ...
              dontPreviewFactory: () => {
                ...
                const fake = createCard({
                  className: 'mbds-w-full mbds-cluster mbds-bg-muted-soft mbds-overflow-hidden',
                });
...
const el = createCarousel({
  ...
  items: [slide1, slide2, slide3, slide4, slide5],
        ...
        const codeVertical = `// A implementação MiniBrain usa orientação horizontal.
...
const el = createCarousel({ items });`;
      ...
      case 'composicoes': {
        ...
        const buildDotsPreview = (): HTMLElement => {
          ...
          dots[0].classList.add('mbds-bg-primary');
        ...
        const buildGalleryPreview = (): HTMLElement => {
          ...
          const items = photos.map((photo) => {
            ...
            coverLabel.className = 'mbds-font-semibold mbds-text-foreground';
            coverLabel.style.fontSize = '1.5rem';
            coverLabel.textContent = photo.title;
            cover.appendChild(coverLabel);

            const body = document.createElement('div');
            body.className = 'mbds-p-4';
            const heading = document.createElement('h3');
            heading.className = 'mbds-text-body mbds-font-semibold mbds-text-foreground';
