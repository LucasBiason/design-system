...
longBody.className = 'mbds-stack mbds-text-body mbds-text-muted-foreground mbds-overflow-y';
...
const drawer = createDrawer({
  ...
  footer,
        ...
        const codeRightPanel = `// No MiniBrain, direction != 'bottom' usa createSheet
import { createSheet } from '@/components/ui/sheet';

const trigger = createButton({ variant: 'outline', label: 'Abrir filtros' });
const body = document.createElement('div');
body.className = 'mbds-px-4 mbds-text-body mbds-text-muted-foreground';
          ...
          tables: [
            {
              ...
              items: [
                ...
                { name: 'open',         type: 'boolean',                     defaultValue: '—',     required: 'Não', description: stripHtml(t('props.table.open.description')) + ' (controlado externamente via .click() no trigger no MiniBrain).' },
