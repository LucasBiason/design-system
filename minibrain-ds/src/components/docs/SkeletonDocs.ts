...
function loadingWrap(label: string, extraClass = '', stackSpacing?: 'xs' | 'sm' | 'md' | 'lg' | 'xl'): HTMLElement {
  ...
  wrap.className = `mbds-w-full ${extraClass}`.trim();
  if (stackSpacing) {
    wrap.classList.add('mbds-stack');
...
function buildCardDemo(label: string): HTMLElement {
  ...
  const inner = loadingWrap('Carregando card de perfil', 'mbds-max-w-sm');
...
function buildListDemo(label: string): HTMLElement {
  const wrap = document.createElement('div');
  wrap.className = 'mbds-w-full mbds-max-w-md mbds-stack';
  wrap.dataset.spacing = 'sm';
  ...
  for (let i = 0; i < 5; i++) {
    ...
    text.className = 'mbds-stack mbds-flex-1';
  ...
  function buildSection(id: SectionId): HTMLElement {
    switch (id) {
          ...
          pairs: [
            {
              ...
              doPreviewFactory: () => {
                const wrap = loadingWrap('Carregando card de perfil', 'mbds-max-w-sm');
                const row = document.createElement('div');
                row.className = 'mbds-cluster';
                row.dataset.spacing = 'md';
                row.appendChild(sizedSkeleton({ height: '3rem', width: '3rem', borderRadius: '9999px' }));
              ...
              dontPreviewFactory: () => {
                const wrap = loadingWrap('Carregando', 'mbds-max-w-sm', 'sm');
      ...
      case 'variantes': {
        ...
        const codeCircle =
          `const avatar = createSkeleton({ height: '3rem', width: '3rem', className: 'mbds-rounded-full' });\n` +
