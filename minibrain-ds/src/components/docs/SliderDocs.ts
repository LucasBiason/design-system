 ...
 *
 * NOTA MiniBrain: o factory custom é wrapper de <input type="range"> nativo —
  ...
  valueText.style.fontVariantNumeric = 'tabular-nums';
            ...
            },
            {
              ...
              previewFactory: () => {
                ...
                note.textContent =
                  'Variante "vertical" não suportada de forma acessível no MiniBrain — use a versão horizontal.';
    ...
    debounceId = setTimeout(() => {
      track('slider_change', { component: 'slider', field_name: 'volume', value: v });
              ...
              previewFactory: () => {
                ...
                status.className = 'mbds-text-caption mbds-text-muted-foreground';
      ...
      case 'propriedades': {
        const interfaceCode = `// createSlider(options) — MiniBrain
export type SliderOptions = {
  ...
  class?: string;
        ...
        return createDocsProps({
          ...
          tables: [
            {
              title: 'createSlider(options) — MiniBrain',
              cols: propsCols,
              items: [
                { name: 'value',         type: 'number',                       defaultValue: 'min',     required: 'Não', description: 'Valor inicial. NOTA MiniBrain: é `number` (não `number[]` como nas libs upstream) — sem suporte a range.' },
                { name: 'onValueChange', type: '(value: number) => void',      defaultValue: '—',       required: 'Não', description: 'Callback disparado durante o arrasto e em cada tecla. NOTA MiniBrain: não existe `onValueCommitted` separado — debounce ou ouça `change` no input nativo.' },
                ...
                { name: 'class',         type: 'string',                       defaultValue: '—',      required: 'Não', description: 'Classes .mbds-* adicionais no `<div>` raiz.' },
              ],
            },
          ],
          interfaceCode,
      ...
      case 'analytics':
        return createDocsAnalytics({
          ...
          items: [
            { event: 'slider_change',        trigger: stripHtml(t('analytics.table.slider_change.trigger')) + ' (MiniBrain: debounce manual sobre onValueChange)', payload: stripHtml(t('analytics.table.slider_change.payload')) },
