          ...
          pairs: [
            ...
            },
            {
              ...
              dontPreviewFactory: () => {
                ...
                li.setAttribute('role', 'menuitem');
                li.className =
                  'mbds-dropdown-menu-item mbds-border-default mbds-rounded-md';
                li.style.minWidth = '160px';
                li.textContent = 'Excluir conta';
                return li;
              },
...
li.textContent = 'Excluir';`;
      ...
      case 'composicoes': {
        function makeLabelItem(text: string): HTMLLIElement {
          ...
          li.className = 'mbds-text-caption mbds-font-semibold mbds-text-muted-foreground';
        ...
        function makeItem(label: string, shortcut?: string): HTMLLIElement {
          ...
          text.className = 'mbds-flex-1';
          text.textContent = label;
          li.appendChild(text);
          if (shortcut) {
            const sc = document.createElement('span');
            sc.className = 'mbds-text-caption mbds-text-muted-foreground';
  ...
  items: [
    ...
    { type: 'item',      label: 'Sair'           },
            ...
            },
            {
              ...
              code: `// MiniBrain: factory padrão não tem checkbox-item; monte manualmente.
const li = document.createElement('li');
li.setAttribute('role', 'menuitemcheckbox');
li.setAttribute('aria-checked', 'true');
li.dataset.state = 'checked';
li.className = 'mbds-cluster relative mbds-px-2 mbds-py-1.5 mbds-text-body mbds-rounded-sm focus:bg-accent';
// indicator + texto…
li.addEventListener('click', () => {
  ...
  li.dataset.state = next ? 'checked' : 'unchecked';
            ...
            },
            {
              ...
              code: `// MiniBrain: monte radio-items manualmente; gerencie exclusividade no click.
function makeRadio(label, checked, group) {
  ...
  return li;
            ...
            },
            {
              ...
              code: `// MiniBrain: factory padrão não exibe shortcut. Adicione um <span aria-hidden>
function makeItem(label, shortcut) {
  ...
  if (shortcut) {
    const sc = document.createElement('span');
    sc.className = 'ml-auto mbds-text-caption tracking-widest mbds-text-muted-foreground';
    sc.setAttribute('aria-hidden', 'true');
    sc.textContent = shortcut;
    li.appendChild(sc);
  }
          ...
          tables: [
            {
              ...
              items: [
                ...
                { name: 'open',         type: 'boolean',                     defaultValue: '—',     required: 'Não', description: stripHtml(t('props.table.open.description')) + ' (controle externo via .click() no trigger no MiniBrain).' },
                { name: 'defaultOpen',  type: 'boolean',                     defaultValue: 'false', required: 'Não', description: stripHtml(t('props.table.defaultOpen.description')) + ' NOTA: factory MiniBrain não tem prop nativa.' },
