...
function buildAlphanumeric(): HTMLElement {
  // Divergência idiomática: factory MiniBrain suporta apenas dígitos.
  ...
  note.className = 'mbds-text-muted-foreground mbds-italic';
  note.style.fontSize = '10px';
  note.textContent = 'MiniBrain: apenas dígitos (fallback)';
          ...
          pairs: [
            {
              ...
              doPreviewFactory: () => {
                const wrap = document.createElement('div');
                wrap.className = 'mbds-text-caption mbds-font-mono mbds-text-muted-foreground';
                wrap.textContent = 'autoComplete="one-time-code"';
                return wrap;
              },
              dontPreviewFactory: () => {
                const wrap = document.createElement('div');
                wrap.className = 'mbds-text-caption mbds-font-mono mbds-text-muted-foreground mbds-italic';
            ...
            },
            {
              ...
              doPreviewFactory: () => {
                const wrap = document.createElement('div');
                wrap.className = 'mbds-text-caption';
                const lbl = document.createElement('p');
                lbl.className = 'mbds-text-caption mbds-font-medium mbds-mb-1';
...
// O factory MiniBrain aceita apenas dígitos (inputMode=numeric, paste filtra \\D).
          ...
          items: [
            ...
            },
            {
              ...
              description:
                ...
                ' (Não suportado pelo factory MiniBrain — fallback numérico.)',
              code: codeAlpha,
      ...
      case 'composicoes': {
        ...
        function applyError(otp: HTMLElement): void {
          ...
          for (const input of inputs) {
            ...
            input.classList.add('mbds-border-destructive', 'ring-destructive/20'); input.style.boxShadow = '0 0 0 2px var(--mbds-ring, currentColor)';
          }
        }
...
help.id = 'otp-help';
help.className = 'mbds-text-caption mbds-text-muted-foreground';
...
otp.querySelectorAll('input').forEach(i => {
  ...
  i.classList.add('mbds-border-destructive');
});

const err = document.createElement('p');
err.id = 'otp-error';
err.className = 'mbds-text-caption mbds-text-destructive';
