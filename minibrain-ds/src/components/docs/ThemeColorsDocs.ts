...
export function createThemeColorsDocs(): HTMLElement {
  ...
  paletteSection.className = 'mbds-stack mbds-docs-section-divider';
  ...
  function createSwatch(token: string): HTMLLIElement {
    ...
    li.className = 'mbds-swatch-grid-item';
  ...
  PALETTE_GROUPS.forEach((group) => {
    ...
    groupWrap.className = 'mbds-swatch-group';

    const groupTitle = document.createElement('h3');
    groupTitle.className = 'mbds-swatch-group-title';
    groupTitleEls.push({ el: groupTitle, key: group.key });

    const grid = document.createElement('ul');
    grid.className = 'mbds-swatch-grid';
  ...
  function createMiniRow(): HTMLElement {
    ...
    row.className = 'mbds-miniswatch-row';
  ...
  brandSection.className = 'mbds-stack mbds-docs-section-divider';
  ...
  brandGrid.className = 'mbds-theme-card-grid';
  ...
  modesSection.className = 'mbds-stack mbds-docs-section-divider';
  ...
  modesGrid.className = 'mbds-theme-card-grid';
  ...
  MODES.forEach((mode) => {
    ...
    const scope = document.createElement('div');
    scope.className = 'mbds-theme-card-scope';
  ...
  axesSection.className = 'mbds-stack mbds-docs-section-divider';
  ...
  axesHeader.className = 'mbds-stack';
  ...
  densityGrid.className = 'mbds-axis-grid';
  ...
  function createDensityTable(): HTMLElement {
    const { wrapper, table } = createTable('mbds-axis-density-table');
  ...
  DENSITY_ITEMS.forEach((item) => {
    ...
    sample.className = 'mbds-axis-sample';

    const label = document.createElement('span');
    label.className = 'mbds-axis-sample-label';
    densityLabelEls.push({ el: label, key: item.key });

    const scope = document.createElement('div');
    scope.className = `mbds-axis-scope ${item.className}`;
  ...
  FONT_ITEMS.forEach((item) => {
    ...
    text.className = 'mbds-font-sample';
  ...
  function syncThemeState() {
    ...
    brandScopeEls.forEach(({ el, className }) => {
      el.className = `mbds-theme-card-scope ${className}${isDark ? ' dark' : ''}`;
    });

    // Cards light/dark: cada um mostra o tema ATIVO num modo fixo.
    modeScopeEls.forEach(({ el, dark }) => {
      el.className = `mbds-theme-card-scope ${brandClass}${dark ? ' dark' : ''}`;
    });
