...
function buildDemoCommand(placeholder: string, withGroups = true): HTMLElement {
  ...
  wrap.className = 'mbds-w-full mbds-max-w-sm mbds-border-default mbds-rounded-md mbds-shadow-md';
  wrap.appendChild(
    createCommand({
      placeholder,
      items: withGroups
          ...
          pairs: [
            {
              ...
              doPreviewFactory: () => {
                const wrap = document.createElement('div');
                wrap.className = 'mbds-w-full mbds-max-w-sm mbds-border-default mbds-rounded-md';
                wrap.appendChild(
                  createCommand({
            ...
            },
            {
              ...
              doPreviewFactory: () => {
                ...
                outer.className = 'mbds-stack mbds-p-2';
                outer.dataset.spacing = 'xs';
                outer.style.alignItems = 'flex-start';
                const hint = document.createElement('div');
                hint.className = 'mbds-cluster mbds-w-full mbds-text-body mbds-text-muted-foreground mbds-border-default mbds-rounded mbds-cursor-pointer';
                hint.dataset.spacing = 'xs';
                hint.style.padding = '0.375rem 0.75rem';
                hint.innerHTML = `
                  <span class="mbds-flex-1">${sanitizeHtml(t('demonstration.labels.openPalette'))}</span>
                  <kbd class="mbds-rounded mbds-border-default mbds-bg-muted mbds-text-caption mbds-font-mono mbds-font-semibold" style="display:inline-flex;align-items:center;justify-content:center;padding:0.125rem 0.375rem;">${sanitizeHtml(t('demonstration.labels.shortcutKey'))}</kbd>
                `;
        ...
        return createDocsVariants({
          ...
          items: [
            ...
            },
            {
              ...
              previewFactory: () => {
                const outer = document.createElement('div');
                outer.className = 'mbds-stack';
                outer.dataset.spacing = 'xs';
                outer.style.alignItems = 'flex-start';
                const trigger = document.createElement('button');
                trigger.className = 'mbds-cluster mbds-border-default mbds-rounded mbds-text-body mbds-cursor-pointer mbds-bg-background';
                trigger.dataset.justify = 'between';
                trigger.style.width = '12rem';
                trigger.style.padding = '0.375rem 0.75rem';
                trigger.innerHTML = `<span>${sanitizeHtml(t('demonstration.labels.selectPlaceholder'))}</span><span class="mbds-text-muted-foreground">▼</span>`;
                const popover = document.createElement('div');
                popover.className = 'mbds-border-default mbds-rounded-md mbds-shadow-md mbds-mt-1';
            ...
            },
            {
              ...
              previewFactory: () => {
                ...
                dialog.className = 'mbds-w-full mbds-max-w-sm mbds-border-default mbds-rounded-md mbds-shadow-md';
                dialog.appendChild(
                  createCommand({
...
wrap.className = 'mbds-border-default mbds-rounded-md mbds-shadow-md';
