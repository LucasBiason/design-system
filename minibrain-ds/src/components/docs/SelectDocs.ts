...
/**
 * Constrói um <select> nativo (factory custom MiniBrain = wrapper de <select>)
          ...
          items: [
            ...
            },
            {
              ...
              description:
                ...
                ' NOTA: o factory `createSelect` (MiniBrain) é um wrapper do `<select>` nativo — o HTML não permite ícones inline em `<option>`. Para essa variante, recomendamos `Combobox` ou um componente custom.',
...
form.addEventListener('submit', (e) => {
  ...
  console.log('Estado:', data.get('state'));
              ...
              previewFactory: () => {
                ...
                const submit = document.createElement('button');
                submit.type = 'submit';
                submit.className = 'mbds-rounded-md mbds-bg-primary mbds-text-body mbds-font-medium';
      ...
      case 'estados':
        return createDocsStates({
          ...
          items: [
            ...
            { label: t('states.items.open'),     trigger: 'Click ou Enter no trigger',           behavior: stripHtml(t('states.descriptions.open')) + ' (no MiniBrain o popup nativo é controlado pelo navegador).' },
...
export type SelectOptions = {
  ...
  class?: string;
        ...
        return createDocsProps({
          ...
          tables: [
            {
              title: 'createSelect(options) — MiniBrain',
              cols: propsCols,
              items: [
                ...
                { name: 'class',         type: 'string',                        defaultValue: '—',          required: 'Não', description: 'Classes .mbds-* adicionais no `<select>` raiz.' },
              ],
            },
          ],
          interfaceCode,
      ...
      case 'notas':
        return createDocsNotes({
          ...
          items: [
            ...
            { title: '', content: sanitizeHtml(t('notes.item2') + ' <strong>MiniBrain</strong>: o factory é um wrapper de <code>&lt;select&gt;</code> nativo — não há portal customizado; o dropdown é o popup nativo do navegador.') },
            { title: '', content: sanitizeHtml(t('notes.item3') + ' <strong>MiniBrain</strong>: type-ahead é gerenciado pelo próprio <code>&lt;select&gt;</code> do navegador.') },
